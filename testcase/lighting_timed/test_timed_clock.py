...
class TestTimedClockOpen:
    """
    定时开关台测试类

    测试定时开台后的自动关台功能，覆盖不同时长的场景：
    - 1 分钟定时开台 → 自动关台
    - 3 分钟定时开台 → 自动关台
    - 1 小时定时开台 → 自动关台
    - 定时中重复开台 → 异常验证
    ...
    def test_timed_clock_open(self, api_client, auth_context, lighting_resources, hour, label, max_deviation):
        """
        定时开台 → 等待自动关台
    ...
    @mark_priority(1)
    @capture_failure
    def test_duplicate_open_error(self, api_client, auth_context, lighting_resources):
        """
